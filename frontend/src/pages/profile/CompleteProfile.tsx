        ...
        <div className="w-full bg-black/40">
          <div className="max-w-6xl mx-auto px-4 py-10 text-white">
            ...
            <p className="mt-2 text-white/90">
              Vui lòng tải lên CMND/CCCD và Bằng lái xe để xác minh danh tính và hoàn tất hồ sơ.
            </p>
          </div>
        </div>
      </div>
        ...
        <aside className="space-y-6">
              ...
              {REQUIRED_DOCS.map((k) => {
                ...
                return (
                  <li key={k} className="flex items-center gap-2 text-sm">
                    ...
                    <span className={ok ? "text-foreground" : "text-muted-foreground"}>{labelMap[k]}</span>
                  </li>
                );
              })}
            </ul>
          ...
          <div className="rounded-xl border bg-card text-card-foreground shadow p-5">
            ...
            <ul className="text-sm text-muted-foreground space-y-2">
              <li className="flex gap-2"><Info className="h-4 w-4 mt-0.5" /> Ảnh rõ nét, đủ 4 góc, không chói sáng.</li>
              <li className="flex gap-2"><Info className="h-4 w-4 mt-0.5" /> Dung lượng tối đa 5MB, định dạng JPG/PNG.</li>
              <li className="flex gap-2"><Info className="h-4 w-4 mt-0.5" /> Thông tin phải trùng khớp với tài khoản.</li>
            </ul>
          </div>
          ...
          <div className="pt-2">
            ...
            {!allDone && (
              <p className="mt-2 text-xs text-muted-foreground text-center">Vui lòng tải đủ 4 ảnh để tiếp tục.</p>
            )}
          </div>
        </aside>
      </div>

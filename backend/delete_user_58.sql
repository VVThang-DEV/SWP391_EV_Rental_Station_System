-- Fix: Delete user and all related data for user_id = 58
...
BEGIN TRY
    ...
    DELETE FROM user_documents WHERE user_id = 58;
    PRINT 'Deleted from user_documents';
    
    DELETE FROM maintenance_records WHERE staff_id = 58;
    PRINT 'Deleted from maintenance_records';
    
    DELETE FROM handovers WHERE staff_id = 58;
    ...
    DELETE FROM payments WHERE reservation_id IN (
        SELECT reservation_id FROM reservations WHERE user_id = 58
    ) OR user_id = 58;
    PRINT 'Deleted from payments';
    
    -- 3. Delete rentals that belong to this user
    DELETE FROM rentals WHERE user_id = 58;
    PRINT 'Deleted from rentals';
    
    -- 4. Now delete reservations (after payments are deleted)
    DELETE FROM reservations WHERE user_id = 58;
    PRINT 'Deleted from reservations';
    
    -- 5. Finally, delete the user
    DELETE FROM users WHERE user_id = 58;
    PRINT 'Deleted user 58 successfully';
